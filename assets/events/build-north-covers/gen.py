"""Build North cover images.

Luma cover images are SQUARE (1:1), minimum 800x800, and Luma rounds the
corners when it renders them, so nothing important goes near a corner.
Source: https://help.luma.com/p/event-cover-images

Square is the Luma deliverable. The wide 2:1 files are for link previews and
LinkedIn, where a landscape crop is what gets used.
"""
import pathlib, subprocess, os
from PIL import Image

CHROME = "/opt/pw-browsers/chromium-1194/chrome-linux/chrome"

# Canadian maple leaf, centred on the origin in a 140 x 132 box.
LEAF = ("m 0,-60 -11.9,22.2 c -1.35,2.41 -2.59,1.71 -3.75,1.11 L -24,-41.3 l 6.25,33.2 c 1.31,6.06 -2.9,6.06 "
        "-4.97,3.45 l -14.6,-16.3 -2.37,8.3 c -0.27,1.09 -1.48,2.24 -3.3,1.93 l -18.5,-3.89 4.86,17.7 c 1.04,3.92 "
        "1.85,5.54 -1.04,6.57 l -6.56,3.08 31.7,25.7 c 3.15,2.44 3.71,4.06 2.57,7.55 l -2.77,9.11 30,-3.46 c 0.93,0 "
        "1.55,0.53 1.55,1.61 l -1.87,34.1 h 5.61 l -1.08,-34 c -0.02,-1.14 0.51,-1.7 1.44,-1.7 l 30,3.46 -2.77,-9.11 "
        "c -1.14,-3.49 -0.58,-5.11 2.57,-7.55 l 31.7,-25.7 -6.56,-3.08 c -2.89,-1.03 -2.08,-2.65 -1.04,-6.57 l "
        "4.86,-17.7 -18.5,3.89 c -1.82,0.31 -3.03,-0.84 -3.3,-1.93 l -2.37,-8.3 -14.6,16.3 c -2.07,2.61 -6.28,2.61 "
        "-4.97,-3.45 l 6.25,-33.2 -8.34,4.79 c -1.16,0.6 -2.4,1.3 -3.75,-1.11 z")

CSS = """
*{{margin:0;padding:0;box-sizing:border-box}}
html,body{{background:{bg}}}
.canvas{{width:{W}px;height:{H}px;position:relative;overflow:hidden;background:{bg};
  font-family:"Bitstream Charter",Charter,Georgia,"Liberation Serif",serif;color:{ink}}}
.grain{{position:absolute;inset:0;opacity:{grain};pointer-events:none;
  background-image:url("data:image/svg+xml;utf8,<svg xmlns='http://www.w3.org/2000/svg' width='160' height='160'><filter id='n'><feTurbulence type='fractalNoise' baseFrequency='0.85' numOctaves='4'/></filter><rect width='160' height='160' filter='url(%23n)'/></svg>")}}
.bar{{position:absolute;top:0;bottom:0;width:{bar}px;background:#c41230}}
.bar.l{{left:0}} .bar.r{{right:0}}
.stage{{position:absolute;left:0;right:0;top:0;bottom:0;display:flex;flex-direction:column;
  align-items:center;justify-content:center;padding-top:{nudge}px}}
.leaf{{width:{leafw}px;height:{leafw}px;margin-bottom:{leafgap}px}}
h1{{font-size:{title}px;line-height:{tlh};letter-spacing:.005em;font-weight:400;text-align:center;white-space:nowrap}}
.seam{{margin-top:{seamgap}px;width:{seamw}px;height:{seamh}px}}
"""

BODY = """<div class="canvas">
<div class="bar l"></div><div class="bar r"></div>
<div class="stage">
  <svg class="leaf" viewBox="-70 -66 140 132" fill="#c41230"><path d="{leaf}"/></svg>
  <h1>{headline}</h1>
  <svg class="seam" viewBox="0 0 200 86">
    <polygon points="100,4 128,38 72,38" fill="#c41230"/>
    <rect x="0" y="41" width="200" height="3" fill="{seamline}"/>
    <polygon points="100,81 72,47 128,47" fill="{tri2}"/>
  </svg>
</div>
<div class="grain"></div>
</div>"""

def build(name, W, H, headline, theme, **geo):
    html = ("<!doctype html><html><head><meta charset='utf-8'><style>"
            + CSS.format(W=W, H=H, **theme, **geo) + "</style></head><body>"
            + BODY.format(leaf=LEAF, headline=headline, seamline=theme["ink"], tri2=theme["ink"])
            + "</body></html>")
    pathlib.Path(f"{name}.html").write_text(html)
    subprocess.run([CHROME, "--headless", "--disable-gpu", "--no-sandbox", "--hide-scrollbars",
                    "--force-device-scale-factor=2", f"--window-size={W},{H+400}",
                    f"--screenshot={name}-raw.png", f"file://{os.getcwd()}/{name}.html"],
                   capture_output=True)
    Image.open(f"{name}-raw.png").convert("RGB").crop((0, 0, W*2, H*2)).save(f"{name}.png")
    os.remove(f"{name}-raw.png")
    print(f"{name}.png  {Image.open(f'{name}.png').size}  ratio {W/H:.3f}")

LIGHT = dict(bg="#F2ECE1", ink="#16191d", grain=".055")
DARK  = dict(bg="#16191d", ink="#F2ECE1", grain=".05")

# Luma cover. Square, 1200 CSS at 2x = 2400px. Headline stacked, no small type.
SQ = dict(bar=96, leafw=196, leafgap=58, title=214, tlh=".95", seamgap=70, seamw=238, seamh=102, nudge=104)
build("cover-light", 1200, 1200, "BUILD<br>NORTH", LIGHT, **SQ)
build("cover-dark",  1200, 1200, "BUILD<br>NORTH", DARK,  **SQ)

# Link preview and LinkedIn. 2:1.
WIDE = dict(bar=112, leafw=104, leafgap=26, title=168, tlh=".9", seamgap=36, seamw=168, seamh=72, nudge=30)
build("cover-wide-light", 1600, 800, "BUILD NORTH", LIGHT, **WIDE)
build("cover-wide-dark",  1600, 800, "BUILD NORTH", DARK,  **WIDE)
